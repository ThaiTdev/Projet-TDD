class Basket {
  ...
  applyDiscountforProduct(couponArray, product) {
    const matchingCoupon = couponArray.find(
      (coupon) => coupon.idProduit === product.id
    );

    if (matchingCoupon && matchingCoupon.code === "") {
      // Appliquer la remise au produit uniquement s'il existe un coupon valide
      // avec le code vide pour ce produit
      product.price -= matchingCoupon.cout;
    } else {
      console.log("Coupon not applicable for this product.");
    }
  }
